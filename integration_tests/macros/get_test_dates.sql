{% macro get_test_dates() -%}
    {{
        return(
            adapter.dispatch("get_test_dates", "dbt_date_integration_tests")()
        )
    }}
{%- endmacro %}
...
{% macro exasol__get_test_timestamps() -%}
    {# Since Exasol timestamps don't have timezone info and our timezone is UTC,
       both time_stamp and time_stamp_utc should be the same value #}
...
{% macro exasol__get_test_dates() -%}
    select
        ...
        cast('{{ modules.datetime.date(1997, 9, 29) }}' as date) as datetime_date,
        cast(
            '{{ modules.datetime.datetime(1997, 9, 29, 6, 14, 0).strftime("%Y-%m-%d %H:%M:%S") }}'
            as {{ dbt.type_timestamp() }}
        ) as datetime_datetime
