{#
Exasol implementation of get_date_dimension macro.
Generates a comprehensive date dimension table with various date attributes.
#}

{% macro exasol__get_date_dimension(start_date, end_date) %}
    with
        ...
        cast(
            {{
                dbt.dateadd(
                    "day",
                    "-1",
                    dbt.dateadd("month", "1", dbt.date_trunc("month", "d.date_day")),
                )
            }} as date
        ) as month_end_date,
        ...
        cast(
            {{
                dbt.dateadd(
                    ...
                    "-1",
                    dbt.dateadd(
                        "month", "1", dbt.date_trunc("month", "d.prior_year_date_day")
                    ),
                )
            }} as date
        ...
        cast(
            {{
                dbt.dateadd(
                    "day",
                    "-1",
                    dbt.dateadd("year", "1", dbt.date_trunc("year", "d.date_day")),
                )
            }} as date
        ) as year_end_date
